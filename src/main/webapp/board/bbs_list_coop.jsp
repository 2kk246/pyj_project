...
<%
    ...
    int totalNum = dao.countArticles("협동");//DB에 저장된 총 힐링 게시글 숫자
    ...
    if (totalNum > 0) {
        articleList = dao.getArticles(startRow, listSize * currPageNum, "협동");
    }
...
<!-- 게시글 부분 -->
<p class="nameOfBoard">협동 게임 게시판</p>
...
%>
        <a href="bbs_list_coop.jsp?pageNum=<%=i%>"> [<%=i%>] </a>&nbsp;
<%
        }
        ...
        if(startNum > navSize){
%>
        	<a href="bbs_list_coop.jsp?pageNum=<%= (startNum-navSize) %>"> &lt;이전&nbsp;</a> 
<%   
        }
...
%>
            <a href="bbs_list_coop.jsp?pageNum=<%= (endNum+1) %>"> 다음&gt; </a>
<% 
         }
    }
}//else문의 닫힘 괄호
