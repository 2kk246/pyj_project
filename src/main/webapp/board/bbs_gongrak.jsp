...
<%
    ...
    int totalNum = dao.countArticles("공략");//DB에 저장된 총 게시글 숫자
    ...
    if (totalNum > 0) {
        articleList = dao.getArticles(startRow, listSize * currPageNum, "공략");
    }
...
<!-- 게시글 부분 -->
<p class="nameOfBoard">게임 공략 게시판</p>
...
%>
        <a href="bbs_gongrak.jsp?pageNum=<%=i%>"> [<%=i%>] </a>&nbsp;
<%
        }
        ...
        if(startNum > navSize){
%>
        	<a href="bbs_gongrak.jsp?pageNum=<%= (startNum-navSize) %>"> &lt;이전&nbsp;</a> 
<%   
        }
...
%>
            <a href="bbs_gongrak.jsp?pageNum=<%= (endNum+1) %>"> 다음&gt; </a>
<% 
         }
    }
}//else문의 닫힘 괄호
