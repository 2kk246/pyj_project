...
<%
    ...
    int totalNum = dao.countArticles(board);//DB에 저장된 총 게시글 숫자
    ...
    if (totalNum > 0) {
        articleList = dao.getArticles_rec(startRow, listSize * currPageNum, board);
    }
...
<!-- 게시글 부분 -->
<p class="nameOfBoard"><%=boardName %></p>
...
%>
        <a href="bbs_list_rec.jsp?pageNum=<%=i%>"> [<%=i%>] </a>&nbsp;
<%
        }
        ...
        if(startNum > navSize){
%>
        	<a href="bbs_list_rec.jsp?pageNum=<%= (startNum-navSize) %>"> &lt;이전&nbsp;</a> 
<%   
        }
...
%>
            <a href="bbs_list_rec.jsp?pageNum=<%= (endNum+1) %>"> 다음&gt; </a>
<% 
         }
    }
}//else문의 닫힘 괄호
